...
(function (window) {
  ...
  function panelAuth(a) {
    ...
    var fromVsReturnVal = (align.from_vs_returnpath_match !== undefined)
      ? align.from_vs_returnpath_match
      : align.from_matches_return_path;
    var fromReturnMatchGlyph = (fromVsReturnVal !== undefined && fromVsReturnVal !== null)
      ? (fromVsReturnVal ? '✓' : '✕')
      : '—';
  ...
  function panelOrigin(a) {
    ...
    var ispVal = (geo && geo.isp !== undefined && geo.isp !== null) ? geo.isp : origin.isp;
    var asnVal = (geo && geo.asn !== undefined && geo.asn !== null) ? geo.asn : origin.asn;
    var infraVal = (geo && geo.infrastructure_type !== undefined && geo.infrastructure_type !== null) ? geo.infrastructure_type : origin.infrastructure_type;
    var isDcVal = (geo && geo.is_datacenter !== undefined && geo.is_datacenter !== null) ? geo.is_datacenter : origin.is_datacenter;
    var isProxyVal = (geo && geo.is_proxy !== undefined && geo.is_proxy !== null) ? geo.is_proxy : origin.is_proxy;
    var isMobileVal = (geo && geo.is_mobile !== undefined && geo.is_mobile !== null) ? geo.is_mobile : origin.is_mobile;

    var tagsHtml = '';
    if (ispVal) tagsHtml += '<span class="origin-tag">' + esc(ispVal) + '</span>';
    if (asnVal) tagsHtml += '<span class="origin-tag">AS' + esc(asnVal) + '</span>';
    if (infraVal) tagsHtml += '<span class="origin-tag">' + esc(infraVal) + '</span>';
    if (isDcVal) tagsHtml += '<span class="origin-tag" style="color:#fb923c; border-color:rgba(249,115,22,0.4);">datacenter</span>';
    if (isProxyVal) tagsHtml += '<span class="origin-tag" style="color:#f87171; border-color:rgba(239,68,68,0.4);">proxy/vpn</span>';
    if (isMobileVal) tagsHtml += '<span class="origin-tag">mobile</span>';
  ...
  function panelRouting(a) {
    ...
    var isMonotonic = integrity.timestamps_monotonic !== undefined ? integrity.timestamps_monotonic : (integrity.chronological_order !== undefined ? integrity.chronological_order : true);
    var orderText = isMonotonic ? '✓ timestamps in order' : '✕ timestamp anomaly';
  ...
  function panelSignals(a) {
    ...
    var defaultSignals = [
      { label: "SPF/DKIM/DMARC failure", points: 30, triggered: false, evidence: "not triggered" },
      { label: "Datacenter / proxy / hosting infrastructure", points: 25, triggered: false, evidence: "not triggered" },
      { label: "Reply-To domain != From domain", points: 20, triggered: false, evidence: "not triggered" },
      { label: "Broken or backward Received chain", points: 15, triggered: false, evidence: "not triggered" },
      { label: "Suspicious urgency language", points: 10, triggered: false, evidence: "not triggered" }
    ];
      ...
      rowsHtml +=
        '<tr class="signal-row ' + (isTriggered ? 'signal-row--on' : 'signal-row--off') + '">' +
          ...
          '<td class="signal-cell">' +
            '<div class="signal-name">' + esc(s.label || s.name) + '</div>' +
